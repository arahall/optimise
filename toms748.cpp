...
static double get_tolerance(double b, int k, double eps)
{
...
static void bracket(function<double(double)> func, 
					double &a, double &b, double &c, double &fa, double &fb, double &tol, int neps, double eps, 
					double &d, double &fd)
{
	...
	tol = fabs(fb) <= fabs(fa) ? get_tolerance(b, neps, eps) : get_tolerance(a, neps, eps);
}
							...
							double eps, bool debug)
{
	...
	for (int iter = 0; iter < max_iter; ++iter)
	{
		...
		{
			c = a - fa / (fb - fa) * (b - a);
			bracket(func, a, b, c, fa, fb, tol, neps, eps, d, fd);
			if (fa == 0.0 || b - a <= tol)
			{
				break;
			}
		...
		bracket(func, a, b, c, d, fb, tol, neps, eps, d, fd);
		if (fa == 0.0 || b - a <= tol)
		{
			break;
		}
		if (b - a >= mu * (b0 - a0))
		{
			...
			bracket(func, a, b, temp_c, fa, fb, tol, neps, eps, d, fd);
		}
		if (fa == 0.0 || b - a <= tol)
		{
			break;
